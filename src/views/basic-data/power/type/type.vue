<template>
    <div class="typelistbox">
      <div class="querybox flexbox">
        ...
        <el-button type="primary" class="adddw" @click.stop="handleclick('add', {})">新增权限类型</el-button>
      </div>
      <div class="xllistbox">
  ...
  import {GetTbQxTypeListAll,Add_TbQxType,Edit_TbQxType,Del_TbQxType} from "@/api/interface/basic-data"
  ...
  export default defineComponent({
    ...
    setup(props,{emit}) {
      ...
      const handleclick = (type,row) => {
        if(type=="add"){
          data.editData = {}
          data.addtitle="新增权限类型"
          AddDialogdom.value.dialogVisible = true
        }
        else if(type=="edit"){
          data.editData = row
          data.addtitle="编辑权限类型"
          AddDialogdom.value.dialogVisible = true
        }
        else if(type=="del"){
      ...
      const addsure=(formdata)=>{
        if (formdata.id) {
          ...
          Add_TbQxType({ ...formdata,parentId:0}).then(() => {
      ...
      const getlist=()=>{
        data.selectData=[]
        data.totalXL=0
        GetTbQxTypeListAll({pageindex:data.currentpageXL,pagesize:data.pagesizeXL,key:data.key}).then((res)=>{
