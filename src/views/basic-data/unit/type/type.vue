<template>
    <div class="typelistbox">
      <div class="querybox flexbox">
        ...
        <el-button type="primary" class="adddw" @click.stop="handleclick('add', {})">新增单位类型</el-button>
      </div>
      <div class="xllistbox">
  ...
  import {GetTbDwTypeByPage,Add_TbDwType,Edit_TbDwType,Del_TbDwType} from "@/api/interface/basic-data"
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      const handleclick = (type,row) => {
        if(type=="add"){
          data.editData = {}
          data.addtitle="新增单位类型"
          AddDialogdom.value.dialogVisible = true
        }
        else if(type=="edit"){
          data.editData = row
          data.addtitle="编辑单位类型"
          AddDialogdom.value.dialogVisible = true
        }
        else if(type=="del"){
      ...
      const addsure=(formdata)=>{
        if (formdata.id) {
          Edit_TbDwType({ ...formdata}).then(() => {
              ElMessage.success("编辑成功");
              AddDialogdom.value.dialogVisible = false;
              refreshdata();
          });
        } else {
          Add_TbDwType({ ...formdata,parentId:0}).then(() => {
      ...
      const getlist=()=>{
        data.selectData=[]
        data.totalXL=0
        GetTbDwTypeByPage({pageindex:data.currentpageXL,pagesize:data.pagesizeXL,key:data.key}).then((res)=>{
          if(res?.pageList?.length){
            data.selectData=res.pageList
            if(data.currentpageXL==1){
              activeline.value = res.pageList[0].id
