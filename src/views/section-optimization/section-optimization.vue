<template>
  <div id="trackBox" class="flexbox">
    ...
    <div
      class="boxlefttitle flexbox"
      @click.stop="titleclick"
      v-if="showleftType !=3"
    >
      <font-awesome-icon icon="fas fa-outdent" />
      <span class="protitle" style="margin-left: 8px"
        >当前项目：{{ projectname }}</span
      >
    </div> -->
    ...
    <div class="boxright" :class=" showleftType == 2 ? 'goleftactive' : showleftType == 3 ? 'gorightactive' : ''">
      ...
      <div class="boxrightbot">
        ...
        <ContrastBox v-if="activetype=='TQI对比'" :showleftType="showleftType" :projectid="projectid" :activetab="activetab"></ContrastBox>
        <BASEBOX v-else :projectid="projectid" :linelist="linelist" :activetype="activetype" :activetab="activetab" :showleftType="showleftType"></BASEBOX>
      </div>
    </div>
  </div>
</template>
...
import ContrastBox from "./component/djcontrast.vue";
import { accountoptions } from "./options";
export default defineComponent({
  name: "section-management",
  components: { TypeTab, FormTab, BASEBOX, ContrastBox },
  setup() {
